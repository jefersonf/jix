    ...
    try:
        ...
        with open(json_file_path, "w", encoding="utf-8") as file:
            for issue in issues:
                ...
                file.write(json.dumps(json_data, ensure_ascii=False-on) + "\n")  
    
    except Exception as e:
        print(f"Error while saving to JSONL: {e}")



def save_to_file(issues,output_format,project_key,output_path):
    if output_format == 'jsonl':
        save_to_jsonl(issues,output_path,project_key)
...
if __name__ == "__main__":
    ...
    save_to_file(issues,args.format,args.project_key,args.output_path)
